...
#input
path_to_cv<-'output/glm_models/CV_two_class/metabolic_syndrome/'               # path to CV folder : we performed 4 times 5 repeated 5-fold cross validation on sampo server
plot_title<-'20 Repeats of 5-fold Cross Validation for metabolic syndrome'     #title of the box plot

# output
anova_result<-'output/excel_files/ANOVA_metabolic_syndrome.csv'                # file to save
######
setwd(path_to_cv)
...
dat<-arrange(dat)
dat$AO<-'Metabolic Syndrome'
save(dat,file ='output/CV_data/metabolic_syndrome.RData')
#plot                                                                           # color codes
...
#               fill_values = color_values,axxis_breaks = seq(.6,.9,.01),axxis_limits = c(.6,.9)))        
require(ggplot2)
ggplot(dat, aes(x=factor(dat$ind,level=nams), y=dat$values,fill=factor(dat$ind,level=nams))) +
  ...
  scale_y_continuous(breaks = seq(0.67,0.87,0.01), limits = c(0.67,.87)) +
