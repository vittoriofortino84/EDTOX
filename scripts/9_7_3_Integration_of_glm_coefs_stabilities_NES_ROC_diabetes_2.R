...
# input
load('output/glm_models/two_class_glm_models_diabetes2.RData')                       # glm model for disease
model_name<-diabetes_2_two_class                                                      # name of models
path_to_CV_results_for_disease<-'output/glm_models/CV_two_class/diabetes_2/'          # path to CV results
# output
RData_file<-'output/diabetes_coefficients_stabilities_two_class.RData'              # OUTPUT save file name RData
...
rm(list=ls())
# input 
load('output/glm_models/two_class_glm_models_diabetes2.RData')    #GLM coefficients and stabilities
model_name<-diabetes_2_two_class 
load('output/train_set_diabetes_2_binary.RData') #TRAINING SET (nes SCORES)
fgs<-train_set_diabetes_2
disease_positive_label<-"edc_positive_diabetes_mellitus_type_2"                  # disease positive label 
disease_negative_label<-"edc_negative_diabetes_mellitus_type_2"                # disease negative label
glm_stability_file_path<-'output/diabetes_coefficients_stabilities_two_class.RData' # output of previuos step
# output
RData_file<-'output/diabetes_coefficients_stabilities_two_class_NES.RData'            # OUTPUT
## Just the above lines should be changed for each diesease
...
# 3. Performing ROC analysis  as a univariate test and integ --------
rm(list=ls())
# input
load('output/train_set_diabetes_2_binary.RData') #TRAINING SET (nes SCORES)
fgs<-train_set_diabetes_2
disease_positive_label<-"edc_positive_diabetes_mellitus_type_2"                  # disease positive label 
disease_negative_label<-"edc_negative_diabetes_mellitus_type_2"                  # disease negative label
path_to_final_data<-'output/diabetes_coefficients_stabilities_two_class_NES.RData'   # path to final data of glm coefs, stabilites and NES scores
# output
final_Rdata<-'output/diabetes_coefficients_stabilities_two_class_NES_ROC.RData'          # RDATA file
final_excel<-'output/excel_files/final_data_diabetes.csv'                                        # Excel file
## Just the above lines should be changed for each diesease 
...
if(all(names(mean_cof)==names(mean_nes_edc_pos_across_net)) &                            # Checking the order of names before integration 
  ...
  mean_cof_each_pathway<-as.data.frame(list(as.numeric(mean_cof),                        # mean of GLM coefficients across all networks for each pathway
                                            ...
                                            as.numeric(mean_nes_edc_neg_across_net),     # mean of NES for EDCs - across all layers for each pathway
                                            names(mean_cof)))}
...
# 4. For presentation to Janni kubblebeck the pathways were seleceted based on the ROC curve of more than .7--------------------------------------------
rm(list=ls())
#input
all_data_final<-read.csv('output/excel_files/final_data_diabetes.csv',stringsAsFactors = F)
# output
output_file<-'output/excel_files/for_janni_diabetesT2.xlsx'
