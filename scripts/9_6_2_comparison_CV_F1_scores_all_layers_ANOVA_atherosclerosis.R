...
#input
path_to_cv<-'output/glm_models/CV_two_class/atherosclerosis/'     # path to CV folder : we performed 4 times 5 repeated 5-fold cross validation on sampo server
plot_title<-'20 Repeats of 5-fold Cross Validation for Atherosclerosis'  #title of the box plot
# output
anova_result<-'output/excel_files/ANOVA_atherosclerosis.csv'      # file to save change the name for other disesse
######
setwd(path_to_cv)
...
dat<-arrange(dat)
dat$AO<-'Atherosclerosis'
save(dat,file ='output/CV_data/atherosclerosis.RData')
...
#               fill_values = color_values,axxis_breaks = seq(0.87,0.95,0.01),axxis_limits = c(0.87,.95)))
...
ggplot(dat, aes(x=factor(dat$ind,level=nams), y=dat$values,fill=factor(dat$ind,level=nams))) +
  ...
  scale_y_continuous(breaks = seq(0.87,0.95,0.01), limits = c(0.87,.95)) +
